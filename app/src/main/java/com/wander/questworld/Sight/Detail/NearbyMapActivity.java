...
public class NearbyMapActivity extends ActionBarActivity implements AMap.OnInfoWindowClickListener, AMap.OnMarkerClickListener, LocationSource {
...
//      CameraUpdate cameraUpdate = CameraUpdateFactory.changeLatLng(latlng);//仅改变可视区域中心点的坐标，其他属性不变。
        CameraUpdate cameraUpdate = CameraUpdateFactory.newLatLngZoom(new LatLng(list.get(0).getLat(), list.get(0).getLng()),16);//可以改变可视区域的经纬度、缩放级别，并且保留其他属性。
        aMap.moveCamera(cameraUpdate);
    }

    private void loadData() {
        HttpUtils httpUtils = MyHttp.getInstance();
        httpUtils.send(HttpRequest.HttpMethod.GET, urlUtils.getSightNearby(id), new RequestCallBack<String>() {
            @Override
...
//                Log.d("destinations",destinationses.toString());
                if (attractions != null) {
                    ...
                    try {
                        MyDB.getDbUtil().updateAll(attractions);
//                        MyDB.getDbUtil().saveAll(destinationses);
            ...
            public void onFailure(HttpException e, String s) {
                Toast.makeText(NearbyMapActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
            }
        });
    }
...
//        Toast.makeText(this, marker.getId()+"你点击了infoWindow窗口" +marker.getTitle().split(":")[0] , Toast.LENGTH_SHORT).show();
