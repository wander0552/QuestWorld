...
/**
 * Created by wander on 2015/5/26.
 * time 12:40
...
/*        //bitmap为空就不调用center函数
        if (bitmap != null) {
            float xEScale = getWidth() / ((float) bitmap.getWidth());
            float yEScale = getHeight() / ((float) bitmap.getHeight());
...
//        matrix.postScale(0.7f,0.7f);
        ...
        this.setOnTouchListener(new OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                MyZoomImageView.this.setImageMatrix(matrix);
                CheckView();
                return true;
            }
        });
